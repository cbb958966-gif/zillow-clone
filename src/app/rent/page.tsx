              ...
              {[
                ...
                <Link
                  ...
                  className={`px-4 py-2 rounded-xl font-medium transition-all duration-300 ${
                    pathname === link.href || (link.href === '/rent' && pathname === '/rent')
                  ...
                  <div className="flex items-center gap-3">
                    <Link href="/dashboard" className="flex items-center gap-2 text-gray-700 hover:text-[#0066cc]">
                      <div className="w-9 h-9 bg-gradient-to-br from-[#0066cc] to-[#0052a3] rounded-full flex items-center justify-center">
                        <span className="text-white font-semibold text-sm">{user.firstName?.[0]?.toUpperCase() || 'U'}</span>
                      </div>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white border-t border-gray-100 py-4 px-4">
            <div className="flex flex-col space-y-3">
              ...
              <Link href="/rent" className="text-[#0066cc] font-medium py-2">Rent</Link>
              ...
              <Link href="/mortgage" className="text-gray-700 hover:text-[#0066cc] font-medium py-2">Mortgage</Link>
        ...
        <div className="container-main relative z-10">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 px-4 py-2 rounded-full mb-4">
              <span className="text-sm font-medium">🏠 {pagination.total.toLocaleString()} Rentals Available</span>
            </div>
            ...
            <p className="text-lg text-white/70">Browse thousands of rental properties across the country</p>
          </div>
                  ...
                  <div className={`property-card ${viewMode === 'list' ? 'flex flex-col md:flex-row' : ''}`}>
                    <div className={`relative ${viewMode === 'list' ? 'md:w-80 h-56' : 'property-card-image'}`}>
                      ...
                      {property.status === 'ACTIVE' ? (
                        <span className="property-card-badge sale">For Rent</span>
                      ) : property.status === 'PENDING' ? (
                        <span className="property-card-badge" style={{ background: '#f59e0b' }}>Pending</span>
                      ) : (
                        <span className="property-card-badge" style={{ background: '#6b7280' }}>Rented</span>
                      )}
...
export default function RentPage() {
  return <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#0066cc]"></div></div>}><RentContent /></Suspense>
}
